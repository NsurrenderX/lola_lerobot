...
# 双 Segment Summary: 也必须进 LAUNCH_ARGS —— resume_search 重建快照时靠它们还原配置
LAUNCH_ARGS+=(--history_tokenization_mode "${HISTORY_TOKENIZATION_MODE}")
LAUNCH_ARGS+=(--history_summary_num_heads "${HISTORY_SUMMARY_NUM_HEADS}")
LAUNCH_ARGS+=(--max_transition_summary_frames "${MAX_TRANSITION_SUMMARY_FRAMES}")
LAUNCH_ARGS+=(--max_task_summary_frames "${MAX_TASK_SUMMARY_FRAMES}")
LAUNCH_ARGS+=(--transition_summary_drop_rate "${TRANSITION_SUMMARY_DROP_RATE}")
LAUNCH_ARGS+=(--task_summary_drop_rate "${TASK_SUMMARY_DROP_RATE}")
LAUNCH_ARGS+=(--history_chain_position_mode "${HISTORY_CHAIN_POSITION_MODE}")
LAUNCH_ARGS+=(--history_chain_max_position "${HISTORY_CHAIN_MAX_POSITION}")
LAUNCH_ARGS+=(--history_summary_last_gate_init "${HISTORY_SUMMARY_LAST_GATE_INIT}")
LAUNCH_ARGS+=(--history_summary_total_length_cap "${HISTORY_SUMMARY_TOTAL_LENGTH_CAP}")
LAUNCH_ARGS+=(--seed "${SEED}")
if [ "$HISTORY_SUMMARY_LAST_CHUNK_RESIDUAL" = false ]; then
    LAUNCH_ARGS+=(--no_history_summary_last_chunk_residual)
fi
if [ "$TRANSITION_SUMMARY_LENGTH_ENCODING" = true ]; then
    LAUNCH_ARGS+=(--transition_summary_length_encoding)
fi
if [ "$TASK_SUMMARY_LENGTH_ENCODING" = false ]; then
    LAUNCH_ARGS+=(--no_task_summary_length_encoding)
fi
